...
SRC_URI="
	x86?   ( http://get.popcorntime.io/build/Popcorn-Time-0.3.8-0-Linux-32.tar.xz )
	amd64? ( http://get.popcorntime.io/build/Popcorn-Time-0.3.8-0-Linux-64.tar.xz )"
...
{
	mkdir -p "${S}" || die
	pkg="${DISTDIR}/Popcorn-Time-0.3.8-0-Linux-$(use x86 && printf "32" || printf "64").tar.xz"
	tar -xf "${pkg}" -C "${S}" || die
}
