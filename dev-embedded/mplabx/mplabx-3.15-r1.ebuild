...
SRC_URI="http://ww1.microchip.com/downloads/en/DeviceDoc/MPLABX-v${PV}-linux-installer.tar"
...
RDEPEND="${DEPEND}
	system-jre? (
		|| (
			>=dev-java/oracle-jre-bin-1.7.0.67[javafx]
			>=dev-java/oracle-jdk-bin-1.7.0.67[javafx]
		)
	)
...
MPLABCOMMVER="3.16.00"

src_unpack()
{
	mkdir -p "${S}" || die
	tar -xf "${DISTDIR}/MPLABX-v${PV}-linux-installer.tar" -C "${S}" || die
	cd "${S}" || die
	"${S}/MPLABX-v${PV}-linux-installer.sh" --noexec --nolibrarycheck --target . || die
	...
	rm "${S}/MPLABCOMM-v${MPLABCOMMVER}-linux-installer.run" || die
	rm "${S}/MPLABCOMM-v${MPLABCOMMVER}-linux-x64-installer.run" || die

	# no longer needed
	rm "${S}/MPLABX-v${PV}-linux-installer.sh" || die
}
...
{
	chroot_create_jail
	chroot_add_bins sh ln tar gzip find
	chroot_add_libs libc.so.6 libdl.so.2 libm.so.6 libnsl.so.1 libreadline.so.6 \
		libacl.so.1 libattr.so.1 libpthread.so.0 libnss_compat.so.2
	chroot_mv "MPLABX-v${PV}-linux-installer.run" /tmp

	if [ $VERBOSE == 1 ]; then
		echo -e "\n\n\n\n\n\n\n\n\n\nY\n\nY\nY\n\nY\nN\nN\nN\nN\nN\n" > answers || die
		chroot_mv answers /tmp
		installcmd="/tmp/MPLABX-v${PV}-linux-installer.run"
	...
	mv -f "chroot/opt/microchip/mplabcomm/v${MPLABCOMMVER}" chroot/"${MPLABCOMMDIR}" || die
	rm -r "chroot/opt" || die
	...
	ln -s ../../"${MPLABCOMMDIR}"/lib/libSerialAccessLink_3_15.so chroot/usr/$(get_libdir)/libSerialAccessLink_3_15.so || die
	ln -s ../../"${MPLABCOMMDIR}"/lib/libUSBAccessLink_3_16.so chroot/usr/$(get_libdir)/libUSBAccessLink_3_16.so || die
	...
	rm -f "chroot/usr/lib/libSerialAccessLink_3_15.so" || die
	rm -f "chroot/usr/lib/libUSBAccessLink_3_16.so" || die
	rm -f "chroot/${MPLABXDIR}/Uninstall_MPLAB_X_IDE_v${PV}.desktop" || die
	rm -f "chroot/${MPLABXDIR}/Uninstall_MPLAB_X_IDE_v3.dat" || die
	rm -f "chroot/${MPLABXDIR}/Uninstall_MPLAB_X_IDE_v${PV}" || die
	...
	# install prelink files
	dodir /etc/prelink.conf.d
	insinto /etc/prelink.conf.d
	insopts --mode=600
	newins "${FILESDIR}/mplabx-prelink.conf" mplabx.conf
}
...
pkg_postinst()
{
	...
	ewarn "    dev-embedded/xc8-bin"
	ewarn "    dev-embedded/xc16 (GPL version)"
	ewarn "    dev-embedded/xc16-bin"
	ewarn "    dev-embedded/xc32-bin"
}
